...
1. **Source the image manually.** Drop your own file at `recipes-content/<dir>/resources/<slug>.jpg`. The file MUST be at `<recipe-dir>/resources/<slug>.jpg` where `<slug>` matches the recipe's `.md` filename. The build derives the image path from the body's `![](resources/...)` link, so filenames must agree.
...
- **Image sourcing.** Hero images are curated by hand. Drop the file at `<recipe-dir>/resources/<slug>.jpg` and add the markdown `![Title](resources/<slug>.jpg)` link below the H1.
- **Image thumbnails.** Generate with `python scripts/generate-thumbs.py` (from `recipes-content/`); walks every `resources/*.jpg` and writes a 400 px JPEG into the sibling `resources/thumbs/`. `--dir <path>` scopes to one folder.
